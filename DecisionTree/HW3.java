...
public class HW3 {
	...
	public static void main(String[] args) {
		...
		if (mode == 0) {
			mTree.rootInfoGain(mTrainDataSet, mTrainAttributeNames, 0 );
		}else if(mode == 1){
			mTree.print();
		}else if(mode == 2){
			int correct = 0;
			for(ArrayList<Double> instance :mTrainDataSet){
				int predict = mTree.classify(instance);
				System.out.println(predict);
				if(instance.get(mTestAttributeNames.size()).intValue() == predict){
					correct++;
				}
			}
			mTree.printAccuracy(correct, mTrainDataSet.size());
		}else if(mode == 3){
			int correct = 0;
			...
			mTree.printAccuracy(correct, mTestDataSet.size());
		}else{
