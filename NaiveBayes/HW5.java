...
public class HW5 {
	...
	private static double[] evaluate(Label[] yTrue, Label[] yPred) {
		...
		for(int i = 0 ; i < yTrue.length; i++){
			...
			if(yTrue[i] == Label.Negative && yPred[i] == Label.Negative){
					trueNeg++;
			}else if(yTrue[i] == Label.Negative && yPred[i] == Label.Positive){
					falsePos++;
			}else if(yTrue[i] == Label.Positive && yPred[i] == Label.Positive){
					truePos++;
			}else{
					falseNeg++;
			}
		}
		scores[0] = round3(((double)trueNeg + (double)truePos)/ ((double)yTrue.length));
		scores[1] = round3(((double)truePos)/ ((double)truePos + (double)falsePos));
		scores[2] = round3(((double)truePos)/ ((double)truePos + (double)falseNeg));
		return scores;
	}
